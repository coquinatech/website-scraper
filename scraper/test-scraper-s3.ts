...
import { generateArchivePath } from "./storage/index.js";

// Import the mirrorSite function from main.ts (we'll need to extract it)
...
// Test script
async function testScraperWithS3() {
  console.log('Testing full scraper with S3 storage...\n');
  ...
  console.log(`Storage engine: S3Storage`);
  console.log(`Archive path: ${archivePath}`);
  console.log();
  
  try {
    await mirrorSite(targetUrl, storage, archivePath, 0, true); // depth 0 for quick test
    
    // Verify the archive structure
    console.log('\nVerifying archive structure in MinIO...');
    const files = await storage.list(archivePath);
    console.log(`Found ${files.length} files in archive:`);
    files.forEach(file => console.log(`  - ${file}`));
    
    // Try to read the main index.html
    const indexPath = `${archivePath}/example.com/index.html`;
    const exists = await storage.exists(indexPath);
    console.log(`\nMain index.html exists: ${exists}`);
    ...
    console.log('\n✅ Scraper with S3 storage test completed successfully!');
  } catch (error) {
